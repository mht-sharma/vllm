...
inline __device__ void apply_rotary_embedding(
  ...
  for (int i = threadIdx.x; i < nq; i += blockDim.x) {
    ...
    apply_token_rotary_embedding<scalar_t, IS_NEOX>(
        query + token_head, cos_ptr, sin_ptr, rot_offset, rot_dim);
  }
  ...
  for (int i = threadIdx.x; i < nk; i += blockDim.x) {
    ...
    const int rot_offset = i % rot_dim;
    apply_token_rotary_embedding<scalar_t, IS_NEOX>(
        key + token_head, cos_ptr, sin_ptr, rot_offset, rot_dim);
  }
}
...
__global__ void rotary_embedding_kernel(
  ...
  apply_rotary_embedding<scalar_t, IS_NEOX>(query, key, cos_ptr, sin_ptr, head_size, num_heads, num_kv_heads, rot_dim, token_idx, query_stride, key_stride);
